...
class TestSupabaseSync(unittest.TestCase):
    ...
    def test_bootstrap_metadata_must_match_sync_configuration(self):
        with tempfile.TemporaryDirectory() as tmp:
            manifest = self._write_json(tmp, "manifest.json", [{"scene_id": "s"}])
            metrics = self._write_json(tmp, "metrics.json", [{"scene_id": "s"}])
            report = {
            ...
            client = _FakeClient()
            with self.assertRaisesRegex(ValueError, "bootstrap_draws"):
                sync_evaluation_report(
                    report, protocol="NARE", dataset_slug="d", candidate_name="c",
                    manifest_path=manifest, metrics_path=metrics, bootstrap_draws=100,
                    bootstrap_seed=0, client=client)
            self.assertEqual(client.calls, [])

    def test_mixed_manifest_splits_are_rejected_before_registry_write(self):
        with tempfile.TemporaryDirectory() as tmp:
            manifest = self._write_json(tmp, "manifest.json", [
                {"scene_id": "s1", "split": "evaluation"},
                {"scene_id": "s2", "split": "lockbox"},
            ])
            metrics = self._write_json(tmp, "metrics.json", [
                {"scene_id": "s1"}, {"scene_id": "s2"},
            ])
            report = {
                "paired": {"per_scene": [
                    {"scene_id": "s1", "baseline_delta_e00": 2.0, "candidate_delta_e00": 1.0},
                    {"scene_id": "s2", "baseline_delta_e00": 2.0, "candidate_delta_e00": 1.0},
                ]},
                "classification": {"ship_gate_passed": False, "classification": "Exploratory"},
            }
            client = _FakeClient()
            with self.assertRaisesRegex(ValueError, "multiple split"):
                sync_evaluation_report(
                    report, protocol="EAGER", dataset_slug="d", candidate_name="c",
                    manifest_path=manifest, metrics_path=metrics, client=client)
            self.assertEqual(client.calls, [])

    def test_negative_scene_improvement_is_valid_when_candidate_is_worse(self):
        with tempfile.TemporaryDirectory() as tmp:
            manifest = self._write_json(tmp, "manifest.json", [{"scene_id": "s"}])
            metrics = self._write_json(tmp, "metrics.json", [{"scene_id": "s"}])
            report = {
                "paired": {"mean_baseline": 2.0, "mean_candidate": 3.0,
                            "mean_improvement": -1.0, "mean_improvement_pct": -50.0,
                            "per_scene": [{"scene_id": "s", "baseline_delta_e00": 2.0,
                                            "candidate_delta_e00": 3.0, "improvement": -1.0}]},
                "classification": {"ship_gate_passed": False, "classification": "Exploratory"},
            }
            result = sync_evaluation_report(
                report, protocol="EAGER", dataset_slug="d", candidate_name="c",
