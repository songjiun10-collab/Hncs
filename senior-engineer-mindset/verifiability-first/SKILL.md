...
- Concrete case (Hncs): `hybrid_engine`'s `summarize()` is the criterion pinned in advance, not chosen after looking at results — every comparison runs a paired t-test **and** a sign test **and** a bootstrap 95% CI (20000 draws, fixed seed) **and** a drop-one sensitivity check, and a CI straddling zero is defined as inconclusive regardless of the mean. It also requires a null result to clear a positive control before being trusted — two near-misses in that codebase (an X-Trans demosaic path collapse, an `OMP_NUM_THREADS` leak that silently blacked out 75% of every frame at exit code 0) would have been reported as "the knob does nothing" if a positive control hadn't caught them. Three past "decisive wins" in the same repo later turned out to be noise or an environment leak — the rule exists because a good-looking mean has already fooled that project multiple times.
