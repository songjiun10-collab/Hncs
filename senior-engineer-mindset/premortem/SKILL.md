...
- Example from this codebase: nobody planned for a subprocess's *inherited environment* as a failure mode. Hncs once called `darktable-cli` from a subprocess that leaked `OMP_NUM_THREADS` from the parent shell — it silently rendered only 1/4 of every frame, exit code 0, no error. The premortem question that would have caught it isn't "does the external tool fail" but "what does this subprocess inherit that I never set explicitly." Fix: build an explicit `env=` for subprocesses, and check output plausibility, never just the exit code.
