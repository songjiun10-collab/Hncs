...
**External validation:** xAI's Grok Build (now open-source, Apache 2.0, `github.com/xai-org/grok-build`) draws the same append-vs-consolidate line this script enforces, and its actual consolidation code (`crates/codegen/xai-grok-memory/src/dream.rs`, `dream_lock.rs`) is worth reading past the docs summary — the real design corrects the simpler picture the docs alone suggested:
- Consolidation ("dream") isn't purely manual. It's gated (`enabled` config, hours-since-last-run, a minimum session count) and normally fires automatically at natural checkpoints (session end) when the gates pass; the `/dream` command bypasses the *gates* for a manual run, not the underlying lock.
- Concurrency is a PID+mtime lock file, explicitly documented as "best-effort coordination, not mutual exclusion" — a stale lock (dead PID, or age past a threshold) gets reclaimed, and the consolidation operation itself is required to be idempotent so a rare double-acquire race is tolerated rather than prevented. The portable lesson: a periodic-or-on-demand consolidation job behind a plain lock file should be designed idempotent, since even a well-built lock only reduces the race, it doesn't eliminate it.
