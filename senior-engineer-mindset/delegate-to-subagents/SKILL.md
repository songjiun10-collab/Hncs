...
- **Scope permissions per role.** Don't give write access to a subagent that's only supposed to review — when responsibilities split, permissions should split too, so a reviewer can't drift into fixing things mid-review. xAI's Grok Build (open-source) ships this as four named tiers on every subagent spawn (`SubagentCapabilityMode::ReadOnly/ReadWrite/Execute/All`, each filtering the actual tool list the child receives) rather than a binary read/write flag — a real system needed more than two buckets.
...
## Give a recurring role a name — treat it like a colleague, not a tool call

- **A role you delegate to repeatedly deserves an identity**, not a fresh anonymous dispatch every time. xAI's Grok Bot (official, docs.x.ai/grok-bot/overview) states this directly: "a Bot = a single persistent, named agent... a persistent, named teammate with a durable state" — not a chatbot you re-explain things to from zero. The same shift costs nothing here: name the role by what it does (`Bug Reproducer`, `PR Steward`) instead of leaving it as an anonymous `general-purpose` dispatch, and treat that name as an identity you build on across dispatches, not a label on one throwaway call.
- **Continue it, don't reset it** (see "Talking to an agent that's already running" above) — this is the mechanical form of "colleague, not tool": a colleague you worked with yesterday doesn't forget everything overnight just because the conversation ended.
- **Feed corrections back into the identity, not just into one conversation.** When a named role's output needed correcting, write the correction into a `persistent-memory` topic keyed to that role's name (see that skill). The next time you delegate to "Bug Reproducer," it starts already knowing the correction — the same way a real colleague improves from feedback instead of repeating the same mistake with the next person who asks.
- **What this doesn't buy you for free:** Grok Bot's bots are reported to scope-match and hand off tasks to each other without a human routing every request — but that specific mechanism isn't in the official docs directly, only in third-party writeups, so treat it as unverified. Nothing in this session's own tooling does automatic scope-matching regardless — a named role here still needs you (or a coordinator, see "Coordinating a hierarchy" below) to decide each handoff by hand. The metaphor buys identity and memory; it doesn't buy self-organizing delegation.

## Coordinating a hierarchy

If you do reach for a coordinator, treat it as a real design decision, not just "add a middle layer":

- **Depth limit: two levels, no more.** You → coordinator → workers. No coordinator-of-coordinators — past that, the reporting chain gets harder to verify than just staying flat would have been. Not just house style: xAI's Grok Build (open-source) enforces a nesting-depth limit mechanically at spawn time (`SubagentDepthCounter` checked against a configured `MaxSubagentDepth`), not as a convention someone has to remember.
...
- **Handing off to a *different* named agent needs a shared place to leave context, not just the message body.** Grok Bot's bots share one persistent computer specifically so a handoff doesn't require re-explaining ("Bots share one computer... makes handoffs possible without repeating setup" — docs.x.ai/grok-bot/overview, official). Claude Code has no literal shared filesystem across sessions, but the honest equivalent is the same shared git repo (commit the state, push it) or a `persistent-memory` topic file — write what the next agent needs somewhere durable and discoverable, then message them to look there, rather than trying to cram all of it into the message text.
...
- **Even where subagent tooling is sophisticated, live steering is often still absent — check, don't assume.** xAI's Grok Build (open-source, `crates/codegen/xai-grok-tools/src/implementations/grok_build/task/`) has a genuinely rich coordinator protocol behind its `task` tool — spawn, poll-with-timeout query, wait-any/wait-all on a list of subagents, typed cancel (by id, by turn, by session, by workflow run), buffered completion summaries delivered between turns — and its message enum is explicitly exhaustive (`SubagentEvent`, doc-commented "kept exhaustive so every actor command is handled"). It has **no message-injection-into-a-running-child variant at all.** `resume_from` only rehydrates a *previously completed* subagent's transcript into a new one — there's no live "send this instruction into the subagent that's still working" path. Don't assume a "continue a running agent" capability exists just because a platform has rich subagent orchestration; the two are separable, and a real, carefully-built implementation shipped without the first. This is scoped to Grok Build's own `task` tool specifically — its sibling product Grok Bot (a different, closed-source architecture: persistent named bots sharing one cloud computer, not ephemeral children of one session) documents the opposite as a headline differentiator ("Bots can collaborate independently, passing context between each other and handing off tasks" / "It coordinates independently with other Bots" — docs.x.ai/grok-bot/overview, official). Not verifiable at the code level like Grok Build was, and "handing off tasks via a shared computer" isn't necessarily the same mechanism as live message-injection — but don't conflate the two xAI products' capabilities just because they share a vendor.
- **Multi-wait is worth naming as an explicit choice**, not just "wait for it": `WaitMode::WaitAny` (proceed as soon as one of several finishes — e.g. racing redundant approaches) vs. `WaitMode::WaitAll` (need every one — e.g. independent slices that all feed the next step) are genuinely different decisions with different failure modes if you default to the wrong one.
...
- **Tag *why* it's stuck, not just that it is.** xAI's Grok Build (open-source, `crates/codegen/xai-workflow/src/run.rs`) types a paused workflow's reason instead of leaving it a bare boolean — `user` (waiting on a human), `back_off` (rate-limited, retry later), `no_progress` (stuck, not converging), `verification` (blocked on a failed check), `infra` (the environment/tooling broke). The fix differs by kind: retry-with-delay for `back_off`, escalate or reslice for `no_progress`, fix the check itself for `verification`, fix the environment for `infra` — collapsing all of these into one generic "BLOCKED" loses exactly the information that says which response applies.
