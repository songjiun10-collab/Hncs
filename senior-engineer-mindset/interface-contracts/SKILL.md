...
- Concrete case (Hncs): every `apply_*` in `brands/*.py` shares one signature — BGR `np.ndarray` in, same-shape out, image first, every other arg defaulted. Nobody wrote that down as an API spec, but `tools/cli/video_engine.py` and `hybrid_engine/` both call any `apply_*` interchangeably on that assumption — that's Hyrum's Law happening in a two-person codebase, not just a big-company thing. The two real deviations (`apply_acros`/`apply_monochrome` in `fuji.py` return a 2D single-channel array, not 3-channel BGR) aren't hidden — they're made an explicit, documented exception and pinned by a golden test, so the deviation can't silently regress back into "looks like every other `apply_*`."
