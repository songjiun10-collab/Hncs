...
- Is this request describing a problem, or a solution already decided on? If it's a solution, why was that solution chosen? — e.g. "make the per-brand calibration scripts share code" can mean two different real problems: centralize the brand-agnostic statistics (`summarize()`, `run_per_brand_calibration()`) that six brand files had actually copy-pasted byte-for-byte, which had already caused a real bug (a hardcoded `datasets/leica/...` path left in 5 of 6 files' error messages) — or couple the per-brand `evaluate_*.py`/`calibrate_profile_<brand>.py` loaders themselves, which are deliberately kept uncoupled (copy the loader, don't import a sibling) so experiments don't tangle as they accumulate. Building the wrong one either recreates a bug that already happened once, or "fixes" duplication that wasn't the actual ask.
