...
- Example from this codebase: Hncs's per-brand calibration scripts deliberately **copy** their data loader instead of sharing it — the simple choice here is duplication, because these scripts are meant to diverge experiment by experiment and a shared abstraction would be solving a problem ("what if loaders need to differ") that doesn't exist yet. But brand-agnostic orchestration code (`summarize()`, `_paired_cv_losses()`) is shared, not copied, because it's meant to stay identical everywhere — sharing is the simple choice there. Same codebase, opposite answer, because the actual variable is "does this need to diverge," not "is DRY always better."
