...
143쌍으로 피팅)의 바디별 분해 - `tools/fit/breakdown_sony_by_camera_body.py`가
Sony a1 II에서 찾은 것과 같은 질문을 Canon에도 물어본 결과
(2026-09-02).

`tools/fit/breakdown_canon_by_camera_body.py`(500px)로 풀링
...
나오는지 `tools/fit/fit_canon_body_split_pipeline.py`로 확인했다(5-fold
...
`python3 -m tools.fit.fit_canon_body_split_pipeline "Canon EOS R6 Mark III"`
(LOO)."""
import colour
import cv2
import numpy as np
