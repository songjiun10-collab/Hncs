...
- 3x3 컬러 매트릭스 단계 - X2D II 41쌍 자체로 새로 피팅해도
  ...
  안 낀 채 음수, `tools/x2dii/evaluate_x2dii_color_matrix.py`) - 매트릭스
  경로는 시도했으나 기각
...
재현: `python3 -m tools.x2dii.evaluate_exposure_gamma_x2dii` (95쌍, X2D II
...
것이다. 재현: `python3 -m tools.x2dii.evaluate_x2dii_generation_loo`
...
tools.x2dii.evaluate_x2dii_generation_loo`(70쌍 기준 매니페스트로 재실행).
...
없었다. `tools/fit/evaluate_full_pixel_de00_confirm.py`(3000px, 다운샘플
...
(`tools/x2dii/evaluate_x2dii_de00_grid.py`, 신규 - 저해상도(200px)로 폴드별
...
갱신**. 재현: `python3 -m tools.x2dii.evaluate_x2dii_de00_grid`.
"""
