...
완전히 동일하다(브랜드 무관 공통 규격, 2026-08부터 `tools/data/build_local_manifest.py`
/`tools/data/verify_contributed_pairs.py`가 `--make` 인자로 제조사를 받도록
...
1. **ColorChecker Classic 24패치 차트** (raw+jpeg 페어, 여러 장 - 조명
   ...
   `tools/fit/analyze_colorchecker_matrix.py`/`hybrid_engine/core/chart_baseline.py`가
...
```
python3 -m tools.data.build_local_manifest ~/Pictures/leica_shoot datasets/leica/contributed/<세트 이름> --make Leica
```

`--make Leica`를 빠뜨리면 기본값(Hasselblad)으로 검증돼서 전부 FAIL
처리된다. `.DNG` 확장자는 이미 `tools/data/build_local_manifest.py`의
`RAW_EXT`에 포함돼 있음(2026-08 추가).

## 검증만 다시 돌리기

```
python3 -m tools.data.verify_contributed_pairs datasets/leica/contributed/<세트 이름> --make Leica
```
