...
## 현재까지의 실측 결론 (v10 기준, `hasselblad_hncs.py` docstring 참고)
...
- raw→JPEG 진짜 전/후 페어(10장)로 그리드서치한 결과, 현재 커브
  구조(toe+shoulder만 있고 전역 노출/감마 단계가 없음)로는 그레이딩
  전/후 밝기 격차를 다 못 메꿔서 자동 탐색값을 그대로 채택하지 않고
  기존 파라미터(`toe_lift=0.001, shoulder_start=0.78, white_point=0.90`)
  유지. 전역 노출 보정 단계 추가가 다음 후보.
