...
- MEDIUM: "상위 에이전트가 허용하면 실행 에이전트가 실행, 단 주의사항
  ...
  <target> :: <caution>` 마커를 파싱하고(모델이 sonnet/opus일 때만
  인정) `medium_approval()`로 소비 가능한 승인을 기록한다 - 자기 자신이
...
def medium_approval(rule, target):
    ...
    (only recorded if that dispatch used sonnet/opus). Unlike the plain
