...
**only** to reproduce `tools/research/evaluate_darktable_vs_rawpy.py`. Nothing else
uses it.
...
== 데이터셋 (전부 .gitignore - 신선한 클론엔 없음) ==
  ...
  OK    downloaded_samples/       123개  - population 샘플 - tools.cli.analyze (없으면 네트워크에서 받음)
...
```
  ...
  PASS  CLI: tools.cli.export_lut --list
  PASS  CLI: tools.cli.export_lut (17격자 .cube 생성)
  PASS  CLI: tools.cli.classify_brand predict
  PASS  CLI: tools.cli.denoise
...
```bash
python3 -m tools.cli.export_lut --list
python3 -m tools.cli.export_lut hasselblad /tmp/hb.cube --size 17
python3 -m tools.cli.classify_brand predict /tmp/hncs_run/_smoke_src.jpg
python3 -m tools.cli.denoise in.jpg out.jpg --strength 5
python3 -m tools.cli.video_engine in.mp4 out.mp4 --brand hasselblad
python3 -m hybrid_engine.convert in.jpg out.jpg --source nikon --target hasselblad
python3 -m tools.cli.raw_pipeline raw_calib_cache/00378.jpg.3FR /tmp/o.tiff --log-space V-Log
```
...
- **`tools.cli.lens_correction` hard-fails the same way** —
  `--focal-length가 없고 EXIF에 FocalLength도 없음`. Pass
  `--focal-length` and `--aperture` explicitly.
- **`tools.cli.analyze <brand>` is not read-only.** It downloads the sample
...
| `--focal-length가 없고 EXIF에 FocalLength도 없음` | Add `--focal-length N --aperture N` to `tools.cli.lens_correction`. |
