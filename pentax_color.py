...
미편집 SOOC JPEG 40장을 모아 population 통계를 냈다 (analyze_pentax_samples.py).
EXIF Make="RICOH IMAGING COMPANY, LTD.", Software가 카메라 펌웨어 버전
문자열("PENTAX 645Z Ver. 1.00" 등)인 것으로 진짜 SOOC 확인. DNG 페어는
라이카/Phase One 때와 마찬가지로 이 사이트에서 못 찾아서 population
통계만 사용.
...
leica_color.py/phaseone_color.py와 동일한 한계: raw 기준선이 없어
population 타깃을 `_film_curve`의 toe_lift/white_point에 직접 대입한
