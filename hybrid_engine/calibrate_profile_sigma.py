...
적용).
  ...
  python3 -m hybrid_engine.calibrate_profile_sigma
"""
import csv
import glob
import math
...
CALIB_MAX_DIM = 500
_SIGMA_CONTRIBUTED_ROOT = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "datasets", "sigma", "contributed")
...
_SIGMA_HUE_LUT_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "assets", "luts", "sigma_hue_learned_scratch.npy")


def _find_pairs():
    pairs = []
    for manifest_path in sorted(glob.glob(os.path.join(_SIGMA_CONTRIBUTED_ROOT, "*", "manifest.csv"))):
...
def _hue_lut_paired_cv_losses(dataset, base_profile, n_folds=4, seed=0):
    ...
    for fold_idx in folds:
        ...
        np.save(_SIGMA_HUE_LUT_PATH, lut)
        hue_profile = dict(base_profile, learned_hue_lut=_SIGMA_HUE_LUT_PATH)
        for i in fold_idx:
...
def run(n_folds=4, seed=0, n_passes=1):
    print("시그마 raw+jpeg 페어 로드 중 (캘리브레이션용 축소 해상도)...")
    dataset = _load_calib_set()
    print(f"총 {len(dataset)}쌍 로드 완료\n")
    if not dataset:
        print("페어를 못 찾음 - datasets/sigma/contributed/*/manifest.csv 확인 필요")
        return

    baseline_loss = _mean_loss(_DEFAULT_PARAMS, dataset)
    print(f"베이스라인 ΔE (V0.1 기본 파라미터, 시그마용 시드 없음): {baseline_loss:.3f}")
