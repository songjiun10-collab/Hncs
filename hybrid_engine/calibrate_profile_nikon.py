...
def _hue_lut_paired_cv_losses(dataset, base_profile, n_folds=4, seed=0):
    ...
    for fold_idx in folds:
        ...
        np.save(_NIKON_HUE_LUT_PATH, lut)
        hue_profile = dict(base_profile, learned_hue_lut=_NIKON_HUE_LUT_PATH)
        for i in fold_idx:
...
def run(n_folds=4, seed=0, n_passes=1):
    print("니콘 raw+jpeg 페어 로드 중 (캘리브레이션용 축소 해상도)...")
    ...
    print(f"베이스라인 ΔE (V0.1 기본 파라미터, 니콘용 시드 없음): {baseline_loss:.3f}")
