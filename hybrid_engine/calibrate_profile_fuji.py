...
데이터 소스만 `datasets/fuji/contributed/*/manifest.csv`로 바꾼다.

`calibrate_profile.py`처럼 프로필 json에 아무것도 쓰지 않는다 - 이 스크립트는
측정만 한다. 실제로 "출시"하려면(후지용 fuji.json을 hybrid_engine
...
if __name__ == "__main__":
    run_per_brand_calibration(
        brand_label_ko="후지",
        dataset_glob_hint="datasets/fuji/contributed/*/manifest.csv",
        load_calib_set=_load_calib_set,
        hue_lut_scratch_path=_FUJI_HUE_LUT_PATH,
    )
