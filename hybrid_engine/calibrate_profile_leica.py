"""[research] 라이카 raw+jpeg 페어로 hybrid_engine 캘리브레이션 - 지금까지
...
측정만 한다. 실제로 "출시"하려면(라이카용 leica.json을 hybrid_engine
...
적용).
  ...
  python3 -m hybrid_engine.calibrate_profile_leica
"""
import csv
import glob
import math
...
CALIB_MAX_DIM = 500
_LEICA_CONTRIBUTED_ROOT = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "datasets", "leica", "contributed")
...
_LEICA_HUE_LUT_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "assets", "luts", "leica_hue_learned_scratch.npy")


def _find_pairs():
    pairs = []
    for manifest_path in sorted(glob.glob(os.path.join(_LEICA_CONTRIBUTED_ROOT, "*", "manifest.csv"))):
...
def _hue_lut_paired_cv_losses(dataset, base_profile, n_folds=4, seed=0):
    ...
    for fold_idx in folds:
        ...
        np.save(_LEICA_HUE_LUT_PATH, lut)
        hue_profile = dict(base_profile, learned_hue_lut=_LEICA_HUE_LUT_PATH)
        for i in fold_idx:
...
def run(n_folds=4, seed=0, n_passes=1):
    print("라이카 raw+jpeg 페어 로드 중 (캘리브레이션용 축소 해상도)...")
    ...
    print(f"베이스라인 ΔE (V0.1 기본 파라미터, 라이카용 시드 없음): {baseline_loss:.3f}")
