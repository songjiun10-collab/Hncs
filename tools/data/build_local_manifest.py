...
<세트>/manifest.csv에 추가하고, tools.data.verify_contributed_pairs의 verify_row로
바로 재검증해서 FAIL 행(대부분 Lightroom/Photoshop 편집본, 또는 셔터 비동기)을
자동으로 제거하는 CLI. manifest.csv에는 항상 PASS한 행만 남는다.

  python3 -m tools.data.build_local_manifest ~/Downloads datasets/hasselblad/contributed/local-mixed-2026-07
  python3 -m tools.data.build_local_manifest ~/Downloads datasets/leica/contributed/<세트> --make Leica
...
def drop_failed(set_dir, expected_make="Hasselblad"):
    ...
    from tools.data.verify_contributed_pairs import verify_row
