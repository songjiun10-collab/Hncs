"""[maintenance] `.claude/hooks/violations_log.jsonl`과`override_audit.jsonl`은
append-only + git-tracked이라 계속 커진다 - 지금은 작지만(11줄), 몇 달 실사용
후엔 파일이 무거워지고 diff 노이즈도 커진다. 이 스크립트는 retention 기간보다
...
오버헤드다. 대신 가끔(예: 월 1회) 수동으로 돌리거나 Routine으로 스케줄.
  ...
  python3 -m tools.maintenance.rotate_hook_logs                      # 기본 두 로그, 90일 보관
  python3 -m tools.maintenance.rotate_hook_logs --retention-days 30
  python3 -m tools.maintenance.rotate_hook_logs path/to/other.jsonl
"""
import argparse
import json
import os
...
_DEFAULT_LOGS = [
    ...
    # tools/maintenance/eval_hook_judgments.py의 _DEFAULT_LEARNING_DATA_LOG와 같은 경로
