"""
tools/research/evaluate_hncs_structural.py의 94쌍(dpreview 클린, 4세대) 재검증이
"방향은 양수지만 CI가 여전히 0을 포함"으로 보류됐던 걸, 이번 세션에
`tools.fit.calibrate.collect_local_pairs()`(2026-08 dedup 수정 후)로 모인
...
4배 늘어난 만큼 폴드당 학습 비용이 커져서 그리드는 줄여 총량 유지).
  ...
  python3 -m tools.research.evaluate_hncs_structural_full_pool
"""
import itertools
import math
import os
...
def load_pairs():
    """tools.fit.calibrate.collect_local_pairs() (dedup 적용됨, 2026-08) - 챠트 프레임
