...
데이터: raw_calib_cache/의 공식 13쌍(tools/fit/calibrate.py의 collect_pairs()가
받아오는 것과 동일 - 없으면 먼저 그 스크립트로 다운로드해야 함).
9쌍은 tools/fit/calibrate.py가 EXIF Software 태그로 걸러낸 편집(Photoshop/
Lightroom) 오염 타깃이라 절대 ΔE 값 자체는 부풀려져 있을 수 있지만,
A/B/C/D 네 변형이 전부 같은 13쌍·같은 타깃을 쓰므로 상대 비교(어느
변형이 이기는지)에는 영향 없다.

  python3 -m tools.research.evaluate_wb_pipeline_variants
"""
import glob
import math
import os
...
def summarize(label_a, des_a, label_b, des_b, n_bootstrap=20000, seed=0):
    ...
    tools/fit/calibrate.py의 summarize()와 같은 통계(부트스트랩 CI, 부호검정) -
