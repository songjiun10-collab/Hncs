...
**왜 만들었나**: `tools/fuji/diagnose_fuji_neutral_render_offset.py`가 후지
GFX50S II Classic Negative 47쌍에서 카메라 JPEG 대비 neutral 렌더가 크게
어둡고(Lab L 중앙값 +74.851, 47/47) 과채도(HSV S -19.490, 0/47)라는 계통
편향을 확인했다. 원인은 `tools/fit/calibrate.py:130`의 `no_auto_bright=True`다.
...
`brands/`나 `tools/fit/calibrate.py`는 읽기만 한다 - 아무것도 수정하지 않는다.
판정은 브랜드별로 부호검정 + 부트스트랩 95% CI(20000회, 고정 시드)로 하고,
CI가 0을 포함하면 그 지표는 판정 보류로 적는다.

  ~/.hncs-hybrid-venv312/bin/python3 -m tools.research.diagnose_neutral_render_offset_by_brand
"""
import csv
import json
import math
...
# 비교 기준 - 후지에서 이미 잰 값 (tools/fuji/diagnose_fuji_neutral_render_offset.py)
...
def main():
    ...
    print(f"  [fuji 기준값] Classic Negative 47쌍 "
          f"(tools/fuji/diagnose_fuji_neutral_render_offset.py):")
    ...
    with open(out, "w", encoding="utf-8") as f:
        json.dump({
            "purpose": "load_neutral_render()의 노출/채도 계통 편향이 후지 "
                       "고유인지 전 브랜드 공통인지 확인",
            "cause_under_test": "tools/fit/calibrate.py:130 no_auto_bright=True",
