...
hybrid_engine/research/hncs_structural.py + tools/research/evaluate_hncs_structural.py
...
전체(디스크에 실제 존재하는 파일만, `tools.fit.calibrate.collect_local_pairs()`)로
...
def load_pairs():
    ...
    (디스크에 실제 존재하는 파일만, tools.fit.calibrate.collect_local_pairs()가
