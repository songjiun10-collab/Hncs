...
python3 -m tools.cli.raw_pipeline photo.CR3 photo.exr --log-space S-Log3   # 32-bit float OpenEXR, scene-referred
...
```
python3 -m tools.cli.export_lut --list                            # list all available presets
python3 -m tools.cli.export_lut hasselblad hasselblad.cube
python3 -m tools.cli.export_lut fuji_astia fuji_astia.cube --size 33   # 33 is the Adobe-standard grid size
python3 -m tools.cli.export_lut hasselblad hasselblad.cube --install-lightroom  # also copy into Lightroom/ACR's LUT Profiles folder
```
...
```
python3 -m tools.cli.analyze hasselblad       # Full population statistics over Hasselblad's official samples
python3 -m tools.cli.analyze portrait         # Portrait subset + skin-tone hue-invariance verification
python3 -m tools.cli.analyze leica            # Leica imaging-resource.com population
python3 -m tools.cli.analyze phaseone         # Phase One, same
python3 -m tools.cli.analyze pentax           # Pentax, same
python3 -m tools.cli.analyze ricoh_gr         # Ricoh GR, same
python3 -m tools.cli.analyze fuji_film_modes  # Population per Fuji Film Mode + preset-direction verification

python3 -m tools.cli.download fuji-links      # Collect Fuji RAW/JPEG Google Drive links
python3 -m tools.cli.download fuji-pairs      # Download RAW+JPEG pairs from those links (requires gdown)

python3 -m tools.fit.calibrate grid_search    # True before/after grid search from Hasselblad raw (requires rawpy, large downloads)
python3 -m tools.fit.calibrate learn_curve    # Learn a tone curve directly from raw+jpeg pixel correspondence (requires rawpy)
python3 -m tools.fit.calibrate regularize     # Regularize the learned LUT + leave-one-out cross-validation
```
