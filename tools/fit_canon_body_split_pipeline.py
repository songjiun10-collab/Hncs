"""
`tools/breakdown_canon_by_camera_body.py`가 확인한 R1(13.914)/R6 Mark
III(16.969) 격차(부트스트랩 95% CI [-4.733,-1.317], 0 미포함)가 바디별
전용 매트릭스+톤+채도/색조로 좁혀지는지 확인한다.
`tools/fit_canon_deployable_pipeline.py`(3x3 매트릭스 최소자승+톤커브
...
톤커브 후보를 구버전과 비교하다 놓쳤던 실수를 반복하지 않기 위해).
  ...
  python3 -m tools.fit_canon_body_split_pipeline <camera_name>
  예: python3 -m tools.fit_canon_body_split_pipeline "Canon EOS R6 Mark III"
"""
import csv
import math
