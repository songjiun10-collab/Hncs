...
population 통계/검증 스크립트 모음 - CLI로 브랜드를 골라 실행한다.
  ...
  python3 -m tools.cli.analyze hasselblad     # 공식 CSV 전수분석 (원래 analyze_all_samples.py)
  python3 -m tools.cli.analyze leica          # imaging-resource.com population (원래 analyze_leica_samples.py)
  python3 -m tools.cli.analyze phaseone       # 〃 (원래 analyze_phaseone_samples.py)
  python3 -m tools.cli.analyze pentax         # 〃 (원래 analyze_pentax_samples.py)
  python3 -m tools.cli.analyze ricoh_gr       # 〃 (원래 analyze_ricoh_gr_samples.py)
  python3 -m tools.cli.analyze fuji_film_modes  # Film Mode별 population + 프리셋 방향 검증 (원래 analyze_fuji_film_modes.py)
  python3 -m tools.cli.analyze portrait       # 인물 서브셋 + 피부톤 hue 불변성 검증 (원래 portrait_skin_analysis.py)
...
from tools.cli.download import list_gallery_images, resolve_full_image, download_file
...
BRAND_CONFIGS = {
    ...
    # 핫셀블라드 표본 확대 시도(2026-07, 실패로 결론) - 기존 124장은 전부
    # cdn.hasselblad.com 공식 갤러리(hasselblad.com/learn/sample-images/)
    # 단일 소스였음. 다른 소스(하셀 공식 인스타그램, explorecams.com/
    # 500px)는 EXIF Software 검증 결과 대부분 편집/재인코딩이라 못 씀
    # (docs/methodology.md 참고). imaging-resource.com의 X2D 100C 리뷰
    # 갤러리(다른 4개 브랜드와 동일 소스, 원본+"-MOD" 편집본 페어 형태)를
    # 실제로 돌려봤더니 비-MOD 후보 45장 전부(44장) "Premature end of
    # JPEG file"로 손상 + 1장은 기대 렌더러 불일치 -> 생존 0장. curl과
    # urllib 두 경로로 동일 URL을 재다운로드해 바이트 단위로 100% 동일하게
    # 깨져 있음을 확인(우리 파이프라인 버그가 아니라 imaging-resource.com
    # CDN 자체의 파일 손상 - Phase One XF 100MP 갤러리 100% 손상 사례와
    # 같은 패턴, docs/methodology.md "이미지 신뢰성 정책" 참고). Phase One
    # XT 때처럼 완전히 못 쓰는 소스라 BRAND_CONFIGS에는 넣지 않고 시도
    # 기록만 남김.
}
...
if __name__ == "__main__":
    ...
    if mode == "hasselblad":
        ...
        print("usage: python3 -m tools.cli.analyze [hasselblad|leica|phaseone|pentax|ricoh_gr|fuji_film_modes|portrait]")
