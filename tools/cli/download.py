...
1. imaging-resource.com 카메라 리뷰 갤러리 공용 스크레이퍼 - 라이카/
   ...
   download_file)을 여기로 합쳤다. tools/cli/analyze.py가 브랜드별
...
사용법: `python3 -m tools.cli.download fuji-links` / `python3 -m tools.cli.download fuji-pairs`
(imaging-resource 엔진은 tools/cli/analyze.py에서 라이브러리로만 쓰고 CLI는 없음)
"""
import csv
import json
import os
...
from tools.data.build_local_manifest import _match_pairs, _parse_dt
...
def download_fuji_pairs():
    ...
    **정정(2026-08)**: 매칭 로직이 이 함수 안에 독자적으로
    ...
    `tools/data/build_local_manifest.py`의 페어 매칭 버그(비결정적/비1:1
...
if __name__ == "__main__":
    ...
    if mode == "fuji-links":
        ...
        print("usage: python3 -m tools.cli.download [fuji-links|fuji-pairs]")
