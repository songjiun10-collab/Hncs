...
RAW/일반 이미지 파일 둘 다 입력으로 받는다.
  ...
  python3 -m tools.cli.lens_correction input.RAF output.jpg
  python3 -m tools.cli.lens_correction input.jpg output.jpg --lens "XF10-24mmF4 R OIS" --focal-length 10 --aperture 8
"""
import argparse
import json
import os
...
def resolve_lens_params(path, *, make=None, model=None, lens=None,
    ...
    두 CLI(`tools.cli.lens_correction`, `tools.cli.raw_pipeline`)가 같은 규칙으로
