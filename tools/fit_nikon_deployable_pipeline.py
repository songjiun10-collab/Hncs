"""
/goal - Canon에서 쓴 것과 같은 방법론(fit_canon_deployable_pipeline.py 참고) -
raw 네이티브 순수 선형(AsShotNeutral 수동 화이트밸런스, libraw 컬러매트릭스
미적용) 입력 대신, 실제 `apply_nikon_look()`이 `load_neutral_render()`가
...
LOO)은 fit_body_matrix_plus_tone_de00.py와 동일, 입력 디코드만 교체.
  ...
  python3 -m tools.fit_nikon_deployable_pipeline [--loo]
"""
import csv
import itertools
import math
...
def load_neutral_render(raw_path, max_dim=MAX_DIM):
    ...
    apply_nikon_look()이 실제로 받는 것과 같은 입력을 재현."""
...
def main():
    ...
    rows = collect_contributed_pairs("nikon", None)
