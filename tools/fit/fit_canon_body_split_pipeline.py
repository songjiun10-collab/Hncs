"""
`tools/fit/breakdown_canon_by_camera_body.py`가 확인한 R1(13.914)/R6 Mark
III(16.969) 격차(부트스트랩 95% CI [-4.733,-1.317], 0 미포함)가 바디별
전용 매트릭스+톤+채도/색조로 좁혀지는지 확인한다.
`tools/fit/fit_canon_deployable_pipeline.py`(3x3 매트릭스 최소자승+톤커브
고정+채도/색조 그리드)를 바디 필터가 되도록 복사(tools/CLAUDE.md "복사
하지 임포트하지 마라" 관례) - 로직은 그대로, `collect_contributed_pairs`
에 `model_filter`만 실제로 연결했다.

각 바디를 자기 데이터만으로 5-fold LOO(매트릭스/채도/색조 매 폴드
재피팅) 하고, **실제 배포된 풀링 `apply_canon_raw_look()`을 같은
held-out 이미지에 직접 돌린 값과 맞대결**한다(Sony a1 II 조사에서
톤커브 후보를 구버전과 비교하다 놓쳤던 실수를 반복하지 않기 위해).

  python3 -m tools.fit.fit_canon_body_split_pipeline <camera_name>
  예: python3 -m tools.fit.fit_canon_body_split_pipeline "Canon EOS R6 Mark III"
"""
import csv
import math
...
def main():
    ...
    rows = collect_contributed_pairs("canon", model_filter)
    print(f"canon [{model_filter}]: manifest {len(rows)}개", flush=True)
    t0 = time.time()
    pairs = []
    ...
    # 실제 배포된 풀링 apply_canon_raw_look()을 이 바디 이미지에 직접 돌린 값 -
    # 진짜 baseline.
    pooled_des = np.array([
        mean_delta_e(bgr_u8_to_linear(apply_canon_raw_look(p['neutral'])), p['target'])
        for p in pairs
    ])
    print(f"풀링 apply_canon_raw_look() 기준 ΔE00 = {pooled_des.mean():.4f}")

    folds = np.array_split(np.random.RandomState(0).permutation(n), N_FOLDS)
    split_des = np.zeros(n)
    for fi, test_idx in enumerate(folds):
        ...
        for i in test_idx:
            ...
            split_des[i] = mean_delta_e(apply_chroma_lut(toned, sat_mult, hue_shift), pairs[i]['target'])
        print(f"  fold {fi+1}/{N_FOLDS} sat={sat_mult:.2f} hue={hue_shift:+.1f}", flush=True)

    diff = pooled_des - split_des
    wins, losses = int((diff > 0).sum()), int((diff < 0).sum())
    mean_pooled, mean_split = float(pooled_des.mean()), float(split_des.mean())
    improvement = (mean_pooled - mean_split) / mean_pooled * 100.0
    rng = np.random.RandomState(0)
    boot = np.array([diff[rng.randint(0, n, n)].mean() for _ in range(20000)])
    ci_lo, ci_hi = np.percentile(boot, [2.5, 97.5])
    p_val = _sign_test_p(wins, losses)
    print(f"\n=== {model_filter} 바디 전용 vs 풀링 apply_canon_raw_look() (n={n}) ===")
    print(f"풀링 ΔE00={mean_pooled:.3f}  바디전용 LOO ΔE00={mean_split:.3f}  개선폭={improvement:+.2f}%")
    print(f"승/패={wins}/{losses}  부호검정 p={p_val:.4f}  부트스트랩 95% CI=[{ci_lo:+.3f},{ci_hi:+.3f}]")
    print("판정:", "보류(CI 0 포함)" if ci_lo <= 0 <= ci_hi else ("바디전용 우세" if improvement > 0 else "풀링 우세"))
