...
`tools.fit.calibrate.load_neutral_render()`(libraw가 **자체 WB+컬러매트릭스를
...
(선형, WB 우회) -> 즉시 다운샘플(`tools.fit.calibrate._resize_to_max_dim`,
...
"카메라 JPEG 근사"고, 최소한 입력 공간만 ICC가 요구하는 것과 맞다.
  ...
  python3 -m tools.fit.fit_brand_native_matrix_for_icc <brand>
"""
import csv
import json
...
from tools.fit.calibrate import _resize_to_max_dim
