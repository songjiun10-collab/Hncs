"""
`tools/fit/evaluate_empirical_tone_curve.py`가 찾아낸 "파라메트릭 film_curve
...
패턴) - 256bin이라도 표본 부족 bin은 선형보간으로 채운다.
  ...
  python3 -m tools.fit.evaluate_learned_lut \
      --label "X2D II 100C" \
...
# tools/fit/fit_final_lut.py의 동일 가드와 값을 맞춤(tools/CLAUDE.md 관례상
...
def _isotonic_regression(y, w):
    """PAVA(pool adjacent violators) - tools/fit/fit_final_lut.py와 동일한
