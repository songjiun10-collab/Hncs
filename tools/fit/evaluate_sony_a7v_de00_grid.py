...
b2/w995 percentile RMSE 기준 그리드서치는 RMSE로는 31.4% 이겼지만 실제
ΔE00은 오히려 -1.12%로 졌다(tools/fit/evaluate_sony_a7v_de00.py,
brands/sony_a7v.py 정정 이력 참고) - 목적함수 자체를 ΔE00으로 바꿔서
같은 문제가 재발하지 않게 한다.

  python3 -m tools.fit.evaluate_sony_a7v_de00_grid
"""
import csv
import math
import os
...
GRID_MAX_DIM = 200
CONFIRM_MAX_DIM = 400
CLAHE_CLIP = 1.25  # 미검증, population-fit 값 차용 - 이번엔 안 건드림
...
CUR_TOE_LIFT, CUR_SHOULDER_START, CUR_WHITE_POINT = 9.1 / 255, 0.78, 228.6 / 255
...
def main():
    ...
    in_sample_best_ci = int(np.argmin(de00.mean(axis=1)))
    tl, ss, wp = COMBOS[in_sample_best_ci]
    print(f"\n=== in-sample 최적(저해상도, ΔE00={de00[in_sample_best_ci].mean():.3f}) ===")
    print(f"toe_lift={tl}, shoulder_start={ss}, white_point={wp}")

    # 기존(population-fit) baseline은 고해상도로 정식 측정
    old_des = []
    for p in pairs:
    ...
    # === LOO (저해상도 행렬로 폴드별 콤보 선택, 평가는 고해상도로 확정) ===
    ...
    print(f"\n=== LOO ΔE00 그리드서치 vs 기존(population-fit) (n={n}) ===")
