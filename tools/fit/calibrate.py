...
골라 실행한다.
  ...
  python3 -m tools.fit.calibrate grid_search       # apply_hncs 파라미터 그리드서치 (원래 calibrate_from_raw.py, v10/v11)
  python3 -m tools.fit.calibrate grid_search_loo   # 위 그리드서치 결과를 leave-one-out으로 과적합 검증 (2026-08)
  python3 -m tools.fit.calibrate grid_search_loo_per_generation  # 풀링 대신 세대별로 따로 피팅했을 때 나아지는지 (2026-08)
  python3 -m tools.fit.calibrate learn_curve       # 파라메트릭 가정 없이 LUT 직접 학습 (원래 learn_tone_curve.py, v12)
  python3 -m tools.fit.calibrate regularize        # 학습 LUT 정규화 + leave-one-out 교차검증 (원래 regularized_lut_loocv.py, 음성 결과)
  python3 -m tools.fit.calibrate regularize_per_generation  # v11 vs v12를 세대 내부에서만 LOO 비교 (2026-08)
...
if __name__ == "__main__":
    ...
    if mode == "grid_search":
        ...
        print("usage: python3 -m tools.fit.calibrate [grid_search|grid_search_loo|"
