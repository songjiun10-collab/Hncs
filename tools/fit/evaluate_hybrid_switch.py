"""
`tools/fit/evaluate_learned_lut.py`가 발견한 "고ISO(노이즈 많은) 사진에서 학습
...
분류 오차 지점을 찾고, held-out 페어에 적용해서 실제 ΔE00을 측정.
  ...
  python3 -m tools.fit.evaluate_hybrid_switch \
      --label "Sony a7V" \
...
MAX_DIM = 250
N_BINS = 256
MIN_BIN_SAMPLES = 30  # tools/fit/fit_final_lut.py와 동일한 가드(2026-08 코드
...
def _isotonic_regression(y, w):
    """PAVA(pool adjacent violators) - tools/fit/fit_final_lut.py와 동일 구현.
