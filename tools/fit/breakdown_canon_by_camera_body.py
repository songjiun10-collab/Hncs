...
(`tools/fit/breakdown_sony_by_camera_body.py`)와 같은 질문: 이 제네릭
함수가 두 바디 중 하나에서 유독 나쁜지 확인한다.

`tools.fit.calibrate.load_neutral_render()`(8비트 BGR, libraw
use_camera_wb=True/gamma=(2.222,4.5)) 디코드 - `apply_canon_raw_look()`
docstring이 명시한 "반드시 8비트 BGR 입력"과 일치.

  python3 -m tools.fit.breakdown_canon_by_camera_body
"""
import csv
import json
import os
