...
**왜 계열을 바꾸나**: `tools/fuji/recalibrate_fuji_classic_negative.py`(현행 계열
...
- `tools/fuji/diagnose_fuji_neutral_render_offset.py`: `load_neutral_render()`
  출력이 카메라 JPEG보다 Lab L 중앙값 기준 크게 어둡다(47/47쌍, 부트스트랩
  95% CI가 0 배제). `tools/fit/calibrate.py:130`의 `no_auto_bright=True` 때문인
  의도된 "무가공 베이스라인"이다.
- `tools/fuji/diagnose_fuji_autobright_vs_look.py`: 그렇다고 auto-bright를 켜도
...
추가할지는 사용자의 별도 결정이다.
  ...
  ~/.hncs-hybrid-venv312/bin/python3 -m tools.fuji.evaluate_fuji_classic_negative_v2_grid
"""
import csv
import json
import math
...
# 검증된 계열의 범위를 그대로 쓴다(tools/fit/evaluate_new_body_de00_grid.py) +
# Classic Negative의 핵심인 채도 축.
TOE_LIFTS = (0.0, 0.02, 0.036, 0.06, 0.09)
# 색공간 버그 수정 후 재실행에서 전체표본 최적이 shoulder_start=0.82(당시
# 상한)로 5/5 만장일치 - 단변량 확인(tools/fuji/probe_fuji_classic_negative_v2_boundary.py)
...
# 과채도인 것(`tools/fuji/diagnose_fuji_neutral_render_offset.py`, 0/47쌍,
