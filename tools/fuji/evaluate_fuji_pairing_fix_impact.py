"""페어 오매칭 정정(`tools/fuji/fix_fuji_manifest_pairing.py`, 2026-09-04)이
...
렌더 경로는 `tools/fuji/evaluate_fuji_preset_de00.py`와 같다
(`load_neutral_render(raw, max_dim=400)` → 룩 → 카메라 JPEG 타깃).

**배포 아님**: `brands/fuji.py`도 프로필도 건드리지 않는다. 측정만 한다.

  ~/.hncs-hybrid-venv312/bin/python3 -m tools.fuji.evaluate_fuji_pairing_fix_impact
"""
import csv
import json
import os
...
# 정정 전 짝 - tools/fuji/fix_fuji_manifest_pairing.py의 SWAP_FIX를 뒤집은 것.
...
def main():
    ...
    report = {
        "question": "페어 오매칭 정정이 후지 룩 측정치를 얼마나 바꿨나 - "
                    "재보정이 필요한지 판단할 근거",
        "fix_commit_context": "tools/fuji/fix_fuji_manifest_pairing.py (2026-09-04) - "
