...
**배경**: `tools/fuji/diagnose_fuji_neutral_render_offset.py`가
`load_neutral_render()` 출력이 카메라 JPEG보다 Lab L 중앙값 기준 크게 어둡고
채도가 높다는 계통 편향을 확인했다(47/47쌍, 부트스트랩 95% CI 0 배제). 원인은
`tools/fit/calibrate.py:130`의 `no_auto_bright=True` - RAF의 노출 헤드룸을 쓰지
...
- ΔE00이 크게 떨어지면: 현행 상수는 멀쩡하고 큰 오차는 렌더 노출 격차였다.
  -> `tools/fuji/recalibrate_fuji_classic_negative.py`가 찾은 "개선"은 룩 상수로
...
`tools/fit/calibrate.py`와 `brands/fuji.py`는 읽기만 한다 - 어느 파일도 수정하지
않는다.

  ~/.hncs-hybrid-venv312/bin/python3 -m tools.fuji.diagnose_fuji_autobright_vs_look
"""
import csv
import json
import math
