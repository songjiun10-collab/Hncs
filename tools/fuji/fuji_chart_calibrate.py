...
tools/cli/analyze.py의 run_fuji_film_modes()는 서로 다른 사진들을 필름모드별로
...
것과 같은 교훈), 이미지 수 자체가 수십 장 규모라 자동화 이득이 적다.
  ...
  python3 -m tools.fuji.fuji_chart_calibrate report        # 실측 delta 테이블 출력 + 저장
  python3 -m tools.fuji.fuji_chart_calibrate grid_search <preset>   # 재보정 그리드서치
"""
import json
import sys
...
if __name__ == "__main__":
    ...
    if mode == "report":
        ...
        print("usage: python3 -m tools.fuji.fuji_chart_calibrate report")
