...
**왜 만들었나**: `tools/fuji/recalibrate_fuji_classic_negative.py`로
...
보정기가 된다). 대상 모드를 인자로 받아 다른 필름모드에도 돌릴 수 있다.
  ...
  ~/.hncs-hybrid-venv312/bin/python3 -m tools.fuji.diagnose_fuji_neutral_render_offset ["Classic Negative"]
"""
import csv
import json
import math
...
def main():
    ...
    report = {
        "purpose": "load_neutral_render() 출력과 카메라 JPEG 사이의 룩과 무관한 "
                   "전역 노출/톤 격차 진단",
        "trigger": "tools/fuji/recalibrate_fuji_classic_negative.py 실행 중 좌표하강이 "
