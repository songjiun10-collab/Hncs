...
`tools/fuji/find_fuji_same_scene_film_mode_groups.py`가 찾은 같은 장면 묶음
...
`tools.fit.calibrate.load_neutral_render(raw, max_dim=400)`
(`tools/fuji/evaluate_fuji_preset_de00.py`와 동일).
...
같은 이름의 `same_scene_film_mode_groups.json`을 그 세트 폴더에서 읽는다.
  ...
  ~/.hncs-hybrid-venv312/bin/python3 -m tools.fuji.evaluate_fuji_film_mode_separation [세트명]
"""
import itertools
import json
import os
...
def main():
    ...
    report = {
        ...
        "input_render": "tools.fit.calibrate.load_neutral_render(raw, max_dim=400) - "
                        "tools/fuji/evaluate_fuji_preset_de00.py와 동일 경로",
