...
RAW -> Log 색공간 -> (선택) LUT 적용 CLI. core/log_pipeline.py 참고.
  ...
  python3 -m tools.raw_pipeline input.CR3 output.tiff --log-space V-Log --auto-expose
"""
import argparse
import os
import sys
...
from core.log_pipeline import (
    ...
    to_log_space, apply_cube_lut, to_16bit_bgr,
)
...
def main():
    ...
    parser.add_argument("--auto-expose", action="store_true",
                         help="평균 밝기 기준 자동노출 (--exposure와 동시 사용 시 자동노출 먼저 적용 후 추가 보정)")
    ...
    if args.auto_expose:
        linear = auto_exposure_average(linear)
