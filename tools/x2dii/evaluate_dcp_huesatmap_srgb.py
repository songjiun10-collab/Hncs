"""
`tools/x2dii/evaluate_dcp_huesatmap.py`(Lab 평면 회전 근사)의 후속 - 실제
...
`tools/x2dii/evaluate_dcp_huesatmap.py`와 같은 원형 가우시안 커널 히스토그램
방식, 같은 진짜 LOO(폴드마다 매트릭스도 재피팅) 방식을 그대로 씀.

  python3 -m tools.x2dii.evaluate_dcp_huesatmap_srgb
"""
import glob
import os
import sys
