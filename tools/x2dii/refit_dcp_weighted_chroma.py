...
`tools/x2dii/evaluate_dcp_weighted_patches.py`가 9장 전체 LOO로 확인한
-4.9%(2.8588->2.7179, 3.5x~5x 구간 평평한 진짜 신호) 근거.

kmichels-x2dii-2026-07 챠트 9장(B_31325~B_31333, B_31334는 manifest에
URL 자체가 없어서 복구 불가 - `tools/x2dii/recover_kmichels_x2dii_chart.py`
참고) 전체로 홀드아웃 없이 최종 배포용 매트릭스를 피팅하고,
`camera_native_matrix_report.json`을 갱신한 뒤
`tools/x2dii/regenerate_x2dii_dcp.py`와 같은 방식으로 .dcp를 재발급한다.

  python3 -m tools.x2dii.refit_dcp_weighted_chroma
"""
import glob
import json
import os
...
def main():
    ...
    report["_comment_weighted"] = (
        ...
        "tools/x2dii/evaluate_dcp_weighted_patches.py 9장 LOO 기준 -4.9%(2.8588->2.7179) "
