"""
`tools/x2dii/evaluate_dcp_huesatmap_srgb.py`가 확인한 sRGB HSV hue-only 보정을
실제 DCP 파일로 만든다 - `tools/x2dii/dcp_export_huesatmap_experimental.py`
...
N=8division/sigma=30(DNG 관례에 가까운 보수적 값 - `tools/x2dii/evaluate_dcp_huesatmap_srgb.py`
...
수준이다.
  ...
  python3 -m tools.x2dii.build_dcp_huesatmap_experimental
"""
import glob
import json
import os
...
from tools.x2dii.evaluate_dcp_irls_weighted import _irls_fit
from tools.x2dii.evaluate_dcp_huesatmap_srgb import (
    _xyz_d50_to_srgb_gamma, _rgb_to_hsv, _fit_hue_table,
)
from tools.x2dii.dcp_export_huesatmap_experimental import write_dcp, read_dcp

SET_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
                        "datasets", "hasselblad", "contributed", "kmichels-x2dii-2026-07")
REPORT_JSON = os.path.join(SET_DIR, "camera_native_matrix_report.json")
OUT_DCP = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
...
def main():
    ...
    report["_comment_huesatmap_experimental"] = (
        ...
        "tools/x2dii/evaluate_dcp_huesatmap_srgb.py 9장 LOO(부트스트랩 CI 없음, "
