...
보류(-5.13%, CI가 0 포함, tools/fit/evaluate_full_pixel_de00_confirm.py)로
나왔다 - Sony a7V 때 겪은 것과 같은 목적함수 문제가 X2D II 핵심
파라미터(exposure_gamma/shoulder_start)에도 있었다는 뜻. ΔE00을 직접
목적함수로 그리드서치를 재검증한다.

  python3 -m tools.x2dii.evaluate_x2dii_de00_grid
"""
import csv
import math
import os
...
from tools.fit.calibrate import load_neutral_render, _grid_search_combos
...
GRID_MAX_DIM = 200
CONFIRM_MAX_DIM = 3000  # tools/fit/evaluate_full_pixel_de00_confirm.py와 동일 상한(OOM 방지)
