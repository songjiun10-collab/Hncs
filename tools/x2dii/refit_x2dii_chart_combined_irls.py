"""`tools/x2dii/refit_x2dii_chart_combined.py`(kmichels 9장 + dpreview 16장,
n=25, 무채색-4x 가중 최소자승만)에 Huber IRLS를 마저 적용한다 - 사용자가
그 결과를 보고 명시적으로 승인("Irls ㄱㄱ").

IRLS 구현은 새로 안 만들고 `tools/x2dii/evaluate_dcp_irls_weighted.py`의
`_irls_fit()`을 그대로 가져다 쓴다(`refit_dcp_irls_final.py`가 kmichels
단독일 때 썼던 것과 같은 재사용 패턴) - 무채색-4x 초기값에서 시작.

  python3 -m tools.x2dii.refit_x2dii_chart_combined_irls
"""
import glob
import json
import os
...
def main():
    ...
    report["_comment_irls"] = (
        ...
        "tools/x2dii/evaluate_dcp_irls_weighted.py의 _irls_fit() 재사용. "
