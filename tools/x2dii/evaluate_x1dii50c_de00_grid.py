"""
apply_hncs_x1d 신설(evaluate_x1d_de00_grid.py) 다음으로, 세대별 분해에서
2위로 나쁜(11.795) X1D II 50C도 같은 방법(exposure_gamma 포함 441콤보
ΔE00 직접 그리드서치, 저해상도 선택 + 3000px 완전 LOO)으로 전용
파라미터가 유의미한지 확인한다. 데이터는
tools.fit.calibrate.collect_local_pairs()(dedup 반영, 세대 라벨
"X1D II 50C", 38쌍).

  python3 -m tools.x2dii.evaluate_x1dii50c_de00_grid
"""
import math
import os
import sys
...
from tools.fit.calibrate import load_neutral_render, collect_local_pairs, _grid_search_combos
...
def main():
    rows = [p for p in collect_local_pairs()
            if p["generation"] == "X1D II 50C" and p["scene_type"] != "chart"]
    print(f"X1D II 50C 페어 {len(rows)}개", flush=True)
    ...
    print(f"\n=== X1D II 50C: LOO ΔE00 그리드서치 vs apply_hncs(main) (n={n}) ===")
