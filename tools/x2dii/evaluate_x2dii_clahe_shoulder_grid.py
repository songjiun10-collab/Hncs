...
(tools/x2dii/evaluate_x2dii_de00_grid.py)가 exposure_gamma/toe_lift/
...
manifest 자체가 이미 브랜드 쪽에서 정리된 124개 페어).
  ...
  python3 -m tools.x2dii.evaluate_x2dii_clahe_shoulder_grid
"""
import csv
import math
import os
...
CONTRIB_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
...
_GS_SHOULDER_STARTS = (0.50, 0.58, 0.66, 0.70, 0.74, 0.78, 0.82)  # tools/fit/calibrate.py 기존 그리드 재사용
