"""tools/dpreview/dpreview_ricoh_griv_links.csv(Ricoh GR IV, dpreview 샘플 갤러리
35쌍 - DNG+JPG)를 실제로 받아서 datasets/ricoh_gr/contributed/griv-dpreview-2026-09/
로 정리하는 원샷 스크립트. ricoh_gr 브랜드는 이 전까지 raw+jpeg 데이터가
0장이었음(population-fit만 존재) - clahe_clip/shoulder_start를 실측
그리드서치로 처음 검증할 수 있게 하는 첫 raw+jpeg 데이터.

JPG는 Cloudflare 차단이 없어서 curl로 바로 받지만, RAW(.dng)는 봇 차단이
걸려있어서 OpenCLI가 붙잡고 있는 실제 로그인 브라우저로 그 URL을
`open`(navigate)해야 진짜 브라우저 다운로드로 ~/Downloads에 떨어진다
(다른 브랜드에서 이미 검증된 패턴, tools/dpreview/download_xcd_lens_gallery.py 참고).

  python3 -m tools.dpreview.download_ricoh_griv_gallery
"""
import csv
import os
import subprocess
...
CSV_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "dpreview_ricoh_griv_links.csv")
SET_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
    "datasets", "ricoh_gr", "contributed", "griv-dpreview-2026-09")
...
def main():
    ...
    writer = csv.DictWriter(manifest_f, fieldnames=[
        "filename_raw", "filename_jpeg", "camera", "iso", "wb_setting",
        "scene_type", "download_url", "notes"])
    if write_header:
        writer.writeheader()
    ...
    for i, row in enumerate(rows):
        ...
        jpg_name, raw_name = row["jpg_name"], row["raw_name"]
        ...
        try:
            ...
            writer.writerow({
                ...
                "camera": meta.get("Model", row["camera"]),
                "iso": meta.get("ISO", ""),
                "wb_setting": meta.get("WhiteBalance", ""),
                "scene_type": "",
                "download_url": raw_url,
                "notes": "dpreview Ricoh GR IV 샘플 갤러리 (L-7050652860)",
            })
            manifest_f.flush()
            ok += 1
