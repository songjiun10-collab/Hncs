...
tools/dpreview/dpreview_x1d_x2d100c_links.csv(X1D 52 + X2D 100C 112 = 164쌍, 원래
61쌍보다 많음 - dpreview 갤러리가 그새 늘어남)를 받아서
`datasets/hasselblad/contributed/x1d-x2d100c-restore-2026-08/`로 정리.
다운로드 메커니즘은 tools/dpreview/download_xcd_lens_gallery.py와 동일 - JPG는 curl,
RAW(.3fr)는 Cloudflare 우회를 위해 OpenCLI 로그인 브라우저로 navigate.

  python3 -m tools.dpreview.download_x1d_x2d100c_restore
"""
import csv
import os
import subprocess
