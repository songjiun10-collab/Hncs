...
작업(`tools/x2dii/refit_dcp_irls_cyan_init.py` 등)과 같은 방법론을 Leica에
...
더 간단한 가중 최소자승만 쓴다.
  ...
  python3 -m tools.dpreview.fit_leica_sl3p_studio_chart <DNG 폴더 경로>
"""
import glob
import json
import os
...
OUT_ICC = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
                        "hybrid_engine", "assets", "profiles", "leica_sl3p_chart.icc")
OUT_REPORT = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
...
def main():
    ...
    report = {
        ...
        "_comment": (
            ...
            "가중 최소자승 - tools/dpreview/fit_leica_sl3p_studio_chart.py 5-fold CV "
