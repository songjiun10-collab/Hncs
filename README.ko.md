...
- `calibrate_profile.py`가 실제 핫셀블라드 raw+jpeg 페어 13쌍으로
  CIEDE2000 ΔE 루프를 돌린다. v1.1(파라메트릭만)은 ΔE00 15.01에서
  정체됐고, 후속 실험 다섯 개(학습 톤 LUT, 학습 hue LUT, 2D/3D 잔차 LUT,
  로컬 콘트라스트 공간 연산)가 전부 교차검증 기준으로는 그걸 못 넘어서
  기각됐다(`hybrid_engine/assets/luts/README.md`에 전체 음성 결과 기록).
  통한 건 따로 있었다 - `--mode raw_baseline`이 색차트 없이 raw 디코드
  ↔ 타깃 JPEG 픽셀 대응만으로 전역 3x3 컬러 매트릭스를 최소자승으로
  맞췄더니(GitHub 이슈 #4 대응) 그것만으로 교차검증 기준 32.6% 개선이
  났다. 문제는 그 매트릭스를 파이프라인에 그대로 끼워 넣자 이득이
  사라졌던 것 - 각 단계를 하나씩 격리해보니 기존의 강제 노출 정규화가
  매트릭스의 이득을 거의 다 없애고 있었다는 걸 찾아 고쳤다. 그 위에서
  톤/채도를 다시 캘리브레이션한(`--mode raw_baseline_pipeline`, nested
  교차검증) 지금 **v1.2**는 ΔE00 15.01 → 9.82(-34.6%, CIE 2000 등급이
  "완전히 다른 색"에서 "한눈에 다름"으로 바뀜)까지 왔다
