...
- **(2026-08 further update)** 169 new GFX50S II pairs landed, verified per film mode - Provia re-confirmed unchanged via a 3-body combined re-check (67 pairs, +19.1%). Classic Chrome had no matching preset at all, so a new one was created (+5.6%). Directly checking the existing Nostalgic Neg preset against raw+jpeg showed it was **actually worse than doing nothing** (-2.1%, wrong direction entirely), so it was re-derived as `apply_nostalgic_neg_v2` (+6.1%) - the old function's code is left untouched for the record, with the correction only appended to its docstring. See [measurements.en.md](measurements.en.md#fuji-gfx50s-ii-large-batch---classic-chrome-added-nostalgic-neg-replaced-2026-08) for details.

## Leica (`brands/leica/look.py`)
