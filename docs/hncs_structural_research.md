...
## 재검증 4의 "딱 하나 어긋난 지점" 해결 - 그리드 크기는 원인이 아니다 (2026-09-03)
...
낮춰서 변수 하나만 격리했다(4클러스터 변형은 이 질문과 무관해서 뺌,
계산량도 줄임). 3워커, 2107초(약 35분) 소요(이 실행 확인된 결과).

**결과(n=389, 256콤보)**:

| 비교 | 개선폭 | 승/패 | 부호검정 p | 부트스트랩 95% CI | 판정 |
|---|---|---|---|---|---|
| 2클러스터하드 vs `apply_hncs()` | -6.57%(10.475→11.163) | 167/222 | 0.0061 | [-0.942,-0.430] | apply_hncs 우세 |
| 블렌딩 vs `apply_hncs()` | -6.71%(10.475→11.178) | 162/227 | 0.0011 | [-0.957,-0.444] | apply_hncs 우세 |
| **블렌딩 vs 2클러스터하드(직접 페어드)** | **-0.13%** | **157/232** | **0.0002** | **[-0.038,+0.009]** | **보류(CI가 0 포함)** |

1024콤보 결과(-6.59%/-6.74%/-0.14%, 승패 167/222·162/227·157/232 -
**승패 숫자가 그리드 크기와 무관하게 정확히 똑같다**, 이 실행
확인된 결과)와 소수점 둘째자리까지 사실상 동일하다 - **그리드를
1024에서 256으로 4배 줄여도 결과가 거의 안 바뀐다는 뜻**이다.
가설이었던 "그리드 4배 차이가 블렌딩 앵커 피팅을 바꿔서 결론이
갈렸다"는 **기각**됐다(이 실행 확인된 결과) - `fit_chroma_lut_grid()`가
1024콤보든 256콤보든 사실상 같은 최적점(sat_mult, hue_shift)에
수렴한다는 뜻이고, 남은 후보 원인은 표본 수 차이(389 vs 364쌍)나
아직 발견 못 한 다른 방법론 차이다(이 세션 스코프 밖, 재확인
