...
- **`MATRIX_RIDGE=1.0`은 사실상 무효다** - 픽셀을 전부 쌓은 정규방정식
  기준 ridge/trace(XᵀX) = 1.2e-5라 ridge=0.0과 계수 차이가 최대 0.16%다.
  기록된 수치는 사실상 정규화 없는 최소자승이며, ridge 값 선택이 결과를
  바꾸지 않는다("정규화로 과적합을 막았다"고 말할 수 없다).
