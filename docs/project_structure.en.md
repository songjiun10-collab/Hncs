...
| `core/lut_export.py` | "Photoshop preset" export - bakes any `brands/*.py` `apply_*` function into a standard Adobe `.cube` 3D LUT by passing the entire identity grid through in one call (`build_identity_grid`/`bake_lut_from_function`/`write_cube_file`). CLAHE-based functions have a documented limitation (a LUT can't structurally represent local adaptivity) noted in the module docstring |
...
| `tools/export_lut.py` | `.cube` 3D LUT export CLI for Photoshop/DaVinci Resolve - `python3 -m tools.export_lut --list` / `python3 -m tools.export_lut hasselblad out.cube [--size 33]` (reuses the `hybrid_engine.core.preset_inverse.TARGET_FUNCS` registry) |
