...
**Architecture:** Add one tool-layer module, `tools/fit/evaluation_common.py`, that owns manifest traversal and the exact existing conversion/DeltaE implementations. The three callers import these functions and retain their existing orchestration, constants, and output paths.
...
**Interfaces:**
- `tools.fit.evaluation_common.collect_contributed_pairs(brand, model_filter=None, film_mode_filter=None)` returns the existing `name/raw_path/jpeg_path` dicts.
...
- [ ] **Step 2: Run the focused tests and verify they fail**
  ...
  Expected: FAIL because `tools.fit.evaluation_common` does not exist yet.
...
- [ ] **Step 4: Replace caller-local definitions with imports**
  ...
  Import the four helpers from `tools.fit.evaluation_common` in each target module,
...
- [ ] **Step 6: Verify repository integrity and diff hygiene**
  ...
  Run: `./.venv/bin/python -m tools.maintenance.audit_repo_integrity` (exit 0 with no
