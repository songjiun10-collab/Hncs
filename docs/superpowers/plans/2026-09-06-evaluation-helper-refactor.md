...
**Goal:** Extract duplicated contributed-pair and color-error helpers from three evaluation tools without changing their CLI behavior or research numbers.

**Architecture:** Add one tool-layer module, `tools/evaluation_common.py`, that owns manifest traversal and the exact existing conversion/DeltaE implementations. The three callers import these functions and retain their existing orchestration, constants, and output paths.

**Tech Stack:** Python 3, `csv`, OpenCV, NumPy, `colour-science`, scikit-image, `unittest`.
...
**Files:**
- Create: `tools/evaluation_common.py`
- Modify: `tools/evaluate_expanded_clahe_shoulder_refit.py`
- Modify: `tools/measure_all_brand_baselines.py`
- Modify: `tools/fit_population_body_de00_grid.py`
- Modify: `docs/project_structure.md`
- Modify: `docs/project_structure.en.md`
- Test: `tests/test_evaluation_common.py`

**Interfaces:**
- `tools.evaluation_common.collect_contributed_pairs(brand, model_filter=None, film_mode_filter=None)` returns the existing `name/raw_path/jpeg_path` dicts.
- `tools.evaluation_common.load_target_linear(jpg_path, shape_hw)` returns resized sRGB-linear float64 RGB.
- `tools.evaluation_common.bgr_u8_to_linear(bgr_u8)` returns sRGB-linear float64 RGB.
- `tools.evaluation_common.mean_delta_e(linear_a, linear_b)` returns the existing scalar CIEDE2000 mean.
...
- [ ] **Step 2: Run the focused tests and verify they fail**
  ...
  Expected: FAIL because `tools.evaluation_common` does not exist yet.

- [ ] **Step 3: Implement `tools/evaluation_common.py`**
...
- [ ] **Step 4: Replace caller-local definitions with imports**
  ...
  Import the four helpers from `tools.evaluation_common` in each target module,
  delete only the duplicate definitions and imports made unused by that
...
- [ ] **Step 6: Verify repository integrity and diff hygiene**
  ...
  Run: `./.venv/bin/python -m tools.audit_repo_integrity` (exit 0 with no
  unlisted-file warnings) and `git diff --check` (no output).

- [ ] **Step 7: Commit the bounded refactor**

  ```bash
  git add tools/evaluation_common.py tools/evaluate_expanded_clahe_shoulder_refit.py tools/measure_all_brand_baselines.py tools/fit_population_body_de00_grid.py tests/test_evaluation_common.py docs/project_structure.md docs/project_structure.en.md docs/superpowers/specs/2026-09-06-evaluation-helper-refactor.md docs/superpowers/plans/2026-09-06-evaluation-helper-refactor.md
