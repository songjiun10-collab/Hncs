...
- **No imports from `tools/*.py`** anywhere under `gui/` (`tools/CLAUDE.md`: "Nothing here is imported by shipped code"). Tabs 2-4 talk to `tools.cli.raw_pipeline`/`tools.cli.lens_correction` only via `subprocess`, never `import`.
...
- Produces: `RAW_EXTS: set[str]` (RAW file extensions, the single copy - tasks 4 and 6 both need this set and import it from here instead of each defining their own), `prepare_for_display(img: np.ndarray, max_width: int = 480) -> PIL.Image.Image`, `quick_raw_preview(path: str) -> np.ndarray` (BGR, uint8), `class ImageView(ttk.Frame)` with `.show(before: np.ndarray, after: np.ndarray) -> None`. Tasks 3-6 import `ImageView` (all), `quick_raw_preview` (tasks 4's RAW branch, 5, 6), and `RAW_EXTS` (tasks 4, 6).
...
**Interfaces:**
- Consumes: `gui.widgets.image_view.ImageView`, `quick_raw_preview`, `RAW_EXTS` (Task 2).
- Produces: `AUTO_LABEL: str`, `is_raw_input(path) -> bool`, `list_jpeg_targets() -> list[str]`, `list_jpeg_sources() -> list[str]`, `list_raw_profiles() -> list[str]`, `build_hybrid_convert_command(input_path, output_path, target, source_override=None, python_exe=None) -> list[str]`, `build_tab(master)`. Appends `("hybrid_engine 변환", hybrid_convert.build_tab)` to `gui.app.TABS`.
