...
- full suite: 1,474 tests passed (rerun on the current checkout)
...
full suite now passes 1,474 tests.
...
Supabase sync also reconciles EAGER baseline/candidate ΔE values with the report, blocking the same numeric substitution attack there.

The Supabase registry now rejects abbreviated git SHAs so execution identity is always bound to a full 40-character commit.
...
Supabase sync rejects registry writes when recorded bootstrap draws/seed differ from the sync configuration.
...
The validated report bytes are reused for the run key and artifact digest, preventing a post-validation path replacement from changing the recorded identity.
