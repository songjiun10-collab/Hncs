...
- 전체 suite: 1,477개 통과 (현재 checkout 재실행)
...
`tests.test_nare_cli`이며 전체 suite는 1,477개 통과했다.
...
Supabase sync는 network write 전에 per-scene ΔE의 유한성·비음수·산술 일관성을 검사해 partial registry write를 막는다.

Supabase sync는 aggregate mean/%도 per-scene에서 재계산해 일치하지 않으면 network write 전에 거부한다.
...
report_path가 없어도 Supabase run_key가 canonical report hash를 포함해 report 내용 변경을 별도 실행으로 식별한다.
...
NARE는 metric 계산 전에 boolean 또는 0 이하 bootstrap 횟수와 boolean seed도 거부한다.

공통 EAGER paired kernel에도 같은 양의 정수 bootstrap 규칙을 적용한다.
